...
const app = new Hono()
    ...
    async (c) => {
        ...
        const exchnage = await client.itemPublicTokenExchange({
        ...
        .values({
            id: createId(),
            userId: auth.userId,
            accessToken: exchnage.data.access_token,
        })
        ...
        const newTransactionsValue = plaidTransactions.data.added
            ...
            if(newTransactionsValue.length > 0) {
                await db
                .insert(transactions)
                .values(newTransactionsValue)
                // .returning();
            }
